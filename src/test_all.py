...
def link_knot_name(folder_name: str) -> str:
    assert re.match(r"\d+\_\d+", folder_name) is not None
    lnum, rnum = [int(v) for v in folder_name.split("_")]
    return "K%da%d" % (lnum, rnum)
...
def main(): # 进行了测试
    for file in list_all_test_file():
        ...
        print("\033[1;34mINFO\033[0m: testing for knot type (\033[1;33m%-5s\033[0m) ... " % knotname_ans, end="") # 输出测试信息
        sys.stdout.flush()
        knotname_got = che_file_to_knot_name.che_file_to_knot_name(file)
        suc_test     = (knotname_ans in knotname_got or ("m" + knotname_ans) in knotname_got) # 成功标志
        print(
